    ...
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl p-4">
          ...
          <p className="text-2xl font-semibold text-stone-800 dark:text-stone-100">{formatINR(balance)}</p>
        </div>
        <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl p-4">
          <p className="text-xs text-stone-400 mb-1">Total Income</p>
          <p className="text-2xl font-semibold text-emerald-700 dark:text-emerald-400">{formatINR(income)}</p>
        </div>
        <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl p-4">
          <p className="text-xs text-stone-400 mb-1">Total Expenses</p>
          <p className="text-2xl font-semibold text-red-600 dark:text-red-400">{formatINR(expenses)}</p>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-1.5 mt-2">
            {categoryData.slice(0, 4).map(({ name, value }) => (
              <div key={name} className="flex items-center justify-between text-xs">
                ...
                <span className="text-stone-700 dark:text-stone-300 font-medium">{formatINR(value)}</span>
              </div>
            ))}
          </div>
        </div>
